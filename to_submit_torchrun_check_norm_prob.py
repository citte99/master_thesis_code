...
training_settings = TrainingSettings(
    ...
    optimizer_args={"weight_decay":1e-4, "betas":(0.9, 0.999) },
    first_lr=0.001,
    following_lr=0.0001,
...
trainer_config = TrainerConfig(
    stages=[ third_stage, fourth_stage], #first_stage, second_stage, third_stage,
...
class Trainer:
    ...
    def _train_stage(self, stage: InputData, index_stage: int, checkpoint=None):
        ...
        lr = self.train_config.training_settings.first_lr*self.world_size if index_stage == 0 else self.train_config.training_settings.following_lr*self.world_size
...
if __name__ == "__main__":
    ...
    trainer = Trainer(
        classifier_name="CHECKNORMSTUFF",
    ...
    trainer = Trainer(
        classifier_name="RUN2clusterEpochPatienceNight",
        train_config=trainer_config,
        local_rank=local_rank,
        gpu_id=local_rank,
        world_size=world_size,
